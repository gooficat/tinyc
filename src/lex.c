...
struct cval_list cvals = {NULL, 0};
struct sym_list syms = {NULL, 0};
...
repeat:
	...
	if (isdigit(buff[col]))
	{
		...
		void *new_els = realloc(cvals.els, ++cvals.len * sizeof *cvals.els);
		if (!new_els)
		{
			// error
			exit(EXIT_FAILURE);
	...
	for (tok.val = 0; tok.val < syms.len; ++tok.val)
	{
		len = strlen(syms.els[tok.val]);
		if (!memcmp(syms.els[tok.val], buff + col, len) &&
			buff[col + len] != '_' && !isalnum(buff[col + len]))
		{
			col += len;
			return;
	...
	void *new_els = realloc(syms.els, ++syms.len * sizeof *syms.els);
	if (!new_els)
	{
		// error
		exit(EXIT_FAILURE);
	...
	memcpy(syms.els[tok.val], buff + col, len);
	syms.els[tok.val][len] = 0;
	col += len;
}
